...
export default function EditAttendancePage() {
  ...
  const fetchDetail = React.useCallback(async () => {
    try {
      ...
      if (response.data?.status === 'success') {
        ...
        setDetail(data)
        setSessionNumber(data.class_session)
        
        if (data.attendance_records && Array.isArray(data.attendance_records)) {
          const initialData: Record<number, number> = {}
          data.attendance_records.forEach((record: AttendanceRecord) => {
            initialData[record.student_id] = Number(record.status)
          })
  ...
  const handleSubmit = async () => {
    ...
    const studentsPayload = detail.attendance_records.map((record) => ({
      student_id: record.student_id,
      student_name: record.student_name,
      status: attendanceData[record.student_id] !== undefined ? attendanceData[record.student_id] : Number(record.status)
    }))

    const payload = {
      class_id: Number(classId),
      class_session: Number(sessionId),
      class_session_new: sessionNumber,
      students: studentsPayload
    }

    try {
      setSubmitting(true)
      const response = await api.put(`/teacher/classes/attendance/${sessionId}`, payload)
      if (response.data?.status === 'success') {
        toast.success(response.data.message || "Cập nhật điểm danh thành công")
        router.push(`/teacher/classes/${classId}/attendance`)
      } else {
        let errorMsg = "Không thể cập nhật điểm danh"
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      <div className="flex justify-end items-center gap-2">
        <Link href={`/teacher/classes/${classId}/attendance/${sessionId}`}>
        ...
        <Button 
          ...
          disabled={submitting || detail.attendance_records?.length === 0}
          ...
          <TableBody>
            {detail.attendance_records?.length === 0 ? (
              ...
              detail.attendance_records?.map((record, index) => {
                const status = attendanceData[record.student_id]
                  ...
                  <TableRow key={record.student_id} className="hover:bg-transparent border-b-border/40 h-14">
                    ...
                    <TableCell className="text-center">
                      <div className="flex justify-center">
                        <Checkbox 
                          checked={status === 0} 
                          onCheckedChange={() => handleStatusChange(record.student_id, 0)}
                    ...
                    <TableCell className="text-center">
                      <div className="flex justify-center">
                        <Checkbox 
                          checked={status === 1} 
                          onCheckedChange={() => handleStatusChange(record.student_id, 1)}
                          className="data-[state=checked]:bg-[#3b82f6] data-[state=checked]:border-[#3b82f6] size-[18px] rounded-sm cursor-pointer"
                        />
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex justify-center">
                        <Checkbox 
                          checked={status === 2} 
                          onCheckedChange={() => handleStatusChange(record.student_id, 2)}
                          className="data-[state=checked]:bg-[#3b82f6] data-[state=checked]:border-[#3b82f6] size-[18px] rounded-sm cursor-pointer"
                        />
                      </div>
                    </TableCell>
