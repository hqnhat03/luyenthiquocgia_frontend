...
export function LoginForm() {
  ...
  // Get email from query params to "persist" form state
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    try {
      ...
      if (result.success || result.status === "success") {
        ...
        if (result.data) {
          tokenStr = result.data.token || result.data.access_token || "";

          if (tokenStr) {
            // Set access_token in cookie temporarily so subsequent API requests are authenticated
            document.cookie = `access_token=${tokenStr}; path=/; max-age=604800; SameSite=Lax`;
          }

          // Fetch complete profile details for student
          try {
            const profileRes = await api.get("/detail");
            const studentProfile = profileRes.data.data;
            userObj = {
              id: result.data.student_id,
              email: studentProfile.email || result.data.account,
              name: (studentProfile.last_name && studentProfile.first_name) ? `${studentProfile.last_name} ${studentProfile.first_name}`.trim() : result.data.account.split("@")[0],
              role: "student",
              avatar: studentProfile.avatar_url || undefined,
            };
          } catch (profileErr) {
            console.error("Error fetching full student profile, falling back to basic info:", profileErr);
            userObj = {
              id: result.data.student_id,
              email: result.data.account,
              name: result.data.account.split("@")[0],
              role: "student",
            };
          }

          if (userObj && tokenStr) {
            useAuthStore.getState().setAuth(userObj, tokenStr);
          }
        }
      ...
      if (error instanceof AxiosError) {
        ...
        if (error.response?.data?.errors) {
          ...
          Object.keys(fieldErrors).forEach((key) => {
            const formKey = key === "account" ? "email" : key;
            form.setError(formKey as keyof LoginFormValues, { message: fieldErrors[key][0] });
          });
        }
      }
    } finally {
      ...
      <div className="flex w-full flex-col justify-center px-6 py-12 md:w-1/2 lg:px-16 xl:px-24">
        <div className="mx-auto flex w-full flex-col justify-center gap-8 sm:w-[420px]">
          <div className="flex flex-col gap-3 text-center md:text-left">
            ...
            <h2 className="text-4xl font-bold tracking-tight">Đăng nhập Học viên</h2>
