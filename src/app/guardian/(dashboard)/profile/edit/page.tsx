...
export default function EditGuardianProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.put("/guardian/profile", formData)
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        ...
        if (formData.name !== authUser?.name) {
          useAuthStore.getState().setUser({ ...authUser!, name: formData.name });
        }
    ...
    <div className="max-w-3xl mx-auto space-y-6 py-8 px-4 md:px-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Chỉnh sửa thông tin cá nhân của bạn.</p>
        </div>
      </div>
