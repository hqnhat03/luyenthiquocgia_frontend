...
export function TeacherSelect({ value = [], onChange, disabled }: TeacherSelectProps) {
  ...
  React.useEffect(() => {
    const fetchTeachers = async () => {
      setIsLoading(true)
      try {
        const res = await api.get("/common/all-teachers")
        
        // Handle variations in API response format (including pagination structure)
        ...
        const formattedTeachers = data.map((t: TeacherType) => {
          const name = t.name || `${t.last_name || ""} ${t.first_name || ""}`.trim() || `ID: ${t.id}`
          teacherDict.current.set(t.id, name)
          return {
            id: t.id,
            name,
            email: t.email,
            expertise: t.subject_names || t.expertise
          }
        })
